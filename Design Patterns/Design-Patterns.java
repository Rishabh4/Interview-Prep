...
Ways to break the singleton pattern?
    1. By implementing Cloneable interface & then cloing the object.
    2. By using Serialization, while restoring the object from memory to actual object.
    3. By using Reflection we can access Constructor and make it accessible.

How To Prevent above ways to break Singleton Object?
    1. Either Throw exception or return same object within the body of clone method.
    2. Implement readResolve and return existing object.
    3. Change the datatype to Enum which doesn't allow to modify anything.
...
What is observer design pattern in Java? When it is used?
    Observer design pattern is one of the behavioral design patterns which defines one-to-many dependencies between objects & is useful
    when we are interested in a state of an object and we want to get notified when there is any change in state of Object.
    In Observer design pattern, when one object changes its state, all its dependent objects are automatically notified,
    the object is called Subject and dependants are called Observers.
    Java provides libraries to implement Observer design pattern using java.util.Observable class & java.util.Observer interface.
...
What is the benefit of using prototype design pattern over creating an instance using the new keyword?⭐️
    Sometimes, object creation is heavyweight and requires a lot of resources, creating a new instance will impact the performance.
    In such cases, a prototype design pattern is used which refers to creating duplicate objects.
    In prototype design pattern, if a similar object is already present then cloning is done keeping performance in mind.
...
What is MVC design pattern ? Give one example of MVC design pattern ?
    ...
    Controller : user request first enters the controller. Controller works between model and view.
...
Difference between strategy and state design pattern in Java?⭐️
    ...
    State Design pattern is useful, in the case of an object of a class that has some or all of its behaviour completely influenced
    by its current state or internal state. And that class is referred to as a Context class.
    A Context object can alter its behaviour when there is a change in its internal state and is also referred as a Stateful object.
...
Can you name few design patterns used in standard JDK library?⭐️
    ...
    BufferedOutputStream decorates OutputStream
    ...
    Proxy
    Hibernate uses proxy to load collection of value types.
    Spring uses proxy pattern to provide support for features like transaction, caching & general AOP.

    Flyweight
    Wrapper classes

    Chain Of Responsibility
    javax.servlet.filter#doFilter(req, res)
