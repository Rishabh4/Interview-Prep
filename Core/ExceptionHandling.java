...
What are Compile Time Errors, Compile Time Exception, Runtime errors and Runtime Exceptions ?⭐️⭐️
   ...
   If you see the exception heirarchy, all the classes are child of Throwable and we have Exception and error.
...
What is difference between Checked and Unchecked Exception in Java ? Explain Exception heirarchy.
    ...
    For example the Exception class has these subclasses:
        IOExceptionclass, AWTException class, SQLException class, InterruptedExceptionclass and RuntimeException Class.
...
How exception is propagated? What is default exception handling?⭐️
    An exception is first thrown from the top of the stack, if it is not caught, it drops down the call stack to the previous method
    and so on until they are caught or until they reach the very bottom of the call stack that means up to main method.
...
What is Exception chaining in Java?
    ...
    of an exception and creating a chain of Exceptions.
...
Does code from finally executes if method returns before finally block or JVM exits ?
