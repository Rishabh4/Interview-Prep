...
L1-00:19 What is MultiThreading and its Purpose?
 ...
 What is Thread? ⭐️
    Thread is separate path of execution in program. Threads are light-weight, They share the same address space.
    Creating thread is simple when compared to process because creating thread requires less resources when compared to process.
...
Which approach is better? ⭐️
    Implementing Runnable is a better approach since we can implement many interfaces but cannot extend multiple classes.

What is Main Thread?
    Thread responsible to execute the main method.

why we call start method? can't we directly call run method? ⭐️
    If we directly call run method, then we are calling a normal method without creating a new thread.
    i.e. run method will behave as normal method and now child thread will not get started
...
Can we override the start method?
    ...
    Thats why you should not override the start method. Still if you want to override the start method,
...
What is Thread Scheduler?
    It is a part of JVM. It is responsible for the order in which the threads get executed
    and there is no assurity of the algorithm used by the scheduler to order the threads. It varies from JVM to JVM
...
Explain the life-cycle of a Thread?
    ...
    Runnable state : A thread can be in the runnable state in either of the following two ways :
        When the start method is invoked or A thread can also be in runnable state after coming back from
        blocked or sleeping or waiting state.

    Running state : If thread scheduler allocates cpu time, then the thread will be in running state.

    Blocking/ Waiting / Sleeping states : In this state the thread can be made temprorily inactive for a short period of time
    ...
    Dead State : A thread is in dead state when thread's run method execution is complete. It dies automatically
        when thread's run method execution is completed and the thread object will be garbage collected.
...
Explain yield() method in thread class ?
    ...
    yield() makes current thread to sleep for a specified amount of time. There is no guarantee that moving
    a current running thread from runnable to running state. It all depends on thread scheduler it doesn't guarantee anything.
...
Explain how join method works?
    ...
    join() method throws Interrupted Exception, so whenever we use join() method
...
Explain about interrupt() method of thread class?
    ...
    throwing InterruptedException. This is reason we need to handle interrupted exception with throws or try/ catch block.
...
What are atomic and non atomic operations? Can volatile make a non-atomic operation to atomic?
    ...
    java provides volatile keyword to make assignment or read or write operations atomic.
  	...
  	but you can't do anything compound such as incrementing it safely, because that's a read/modify/write cycle.
...
What is Synchronization?
	...
	that is only one thread enters this section at at a time and when it exits other thread can enter this section.
...
When a thread is executing synchronized methods, then is it possible to execute other synchronized
methods simultaneously by other threads on the same object?
...
Can we synchronize static methods in java?
    ...
    When a thread was executing static synchronized method, no other thread can execute  static synchronized method of
    class since lock is acquired on class.
...
Explain why wait(), notify() and notifyAll() methods are in Object class rather than in thread class?
  	...
  	these methods are called on an shared object so they are kept in object class rather than thread class.
...
Explain IllegalMonitorStateException and when it will be thrown?
  	...
  	whenever we call these methods, lock is acquired/released on that object so they must be called from within synchronized area.
...
Can we restart a dead thread in java?
  	 If we try to restart a dead thread by using start method, we will get runtime exception since the thread is not alive.

Can one thread block the other thread?
  	No, one thread cannot block the other thread in java. It can block the current thread that is running.
  	for example if we yield a thread..it blocks itself to give chance to other thread..but we can't block other thread.
...
What is Thread Group? Why it's advised not to use it?
    ...
    group so in other words, programmer can group the threads in thread group based on their functions to be performed.
...
What is fork join pool?⭐️
    Java 7 introduced the fork/join framework. It provides tools to help speed up parallel processing by attempting
        to use all available processor cores. It accomplishes this through a divide and conquer approach.
    ...
    After that, the "join" part begins. The results of all subtasks are recursively joined into a single result.
        In the case of a task that returns void, the program simply waits until every subtask runs.
...
What is completable future?
    Asynchronous computation is difficult to reason about. Usually, we want to think of any computation as a series
        of steps, but in the case of asynchronous computation, actions represented as callbacks tend to be either
        scattered across the code or deeply nested inside each other. Things get even worse when we need to handle
...
What is FutureTask Class?
    FutureTask is the base implementation class of Future interface & we can use it with Executors for asynchronous processing.
...
What is CountDownLatch? When we use CountDownLatch?
    A CountDownLatch has a counter field, which you can decrement as we require. We can then use it to block a
        calling thread until it's been counted down to zero.

    If we were doing some parallel processing, we could instantiate the CountDownLatch with the same value for
        the counter as a number of threads we want to work across. Then, we could just call countdown() after each
        thread finishes, guaranteeing that a dependent thread calling await() will block until the worker threads are finished.

What is CyclicBarrier? What is the difference between CountDownLatch and CyclicBarrier?
    CyclicBarrier is used to make threads wait for each other. It is used when different threads process a part of
        computation and when all threads have completed the execution, the result needs to be combined in the parent
        thread. In other words, a CyclicBarrier is used when multiple thread carry out different sub tasks and the
        output of these sub tasks need to be combined to form the final output. After completing its execution, threads
        call await() method and wait for other threads to reach the barrier. Once all the threads have reached, the
        barriers then give the way for threads to proceed.

        CountDownLatch                                                  CyclicBarrier
    It is a construct that a thread looks out for while      A CyclicBarrier is a reusable construct where a gathering of
    different threads tally down on the latch until it       threads stands by together until the entirety of the threads
    arrives at zero.                                         shows up. By then, the barrier is broken and a move can alternatively be made.

    It keeps up a count of tasks.                            It keeps up a count of threads.

    In CountDownLatch single thread can countdown more       In CyclicBarrier single thread can call awaits only once which
    than once, this would reduce count by number of times    would reduce barrier count by one only, even if call
    countdown() method is called.	                         awaits() method more than once.
    When we are using a CountDownLatch, you must specify     When we are using CyclicBarrier, you must specify the no. of
    the no. of calls to the countdown() method while         threads that should call await() function to trip the barrier.
    creating a CountDownLatch object.	

    It is initialized to N used to make one thread stand     If you have a CyclicBarrier initialized to 3 that implies
    by until N strings have finished some activity, or       you ought to have in any event 3 strings to call await().
    some activity has been finished N times.	

    It cannot be reused, when count arrives at zero it       It can be reused after holding threads are released.
    can't be reset. 	

    In CountDownLatch just the current thread that has       In a CyclicBarrier, if a thread experiences an issue (timeout,
    an issue throws a special case/exception.	             interruption), the wide range of various threads that have 
                                                             reached await() get a special case/exception.

    It's advanceable.	                                     It's not advanceable.

    If the current thread is interrupted, it will            If one thread is interrupted while waiting then all other
    throw InterruptedException. It will not impact           waiting threads will throw BrokenBarrierException.
    other threads.
...
Briefly explain Semaphore.
    A semaphore is an integer variable, shared among multiple processes. The main aim of using a semaphore is to
        process synchronization and access control for a common resource in a concurrent environment.
