...
How many methods Serializable has? If no method then what is the purpose of Serializable interface?
  	It is a marker interface. The main purpose of using marker interface is to tell the compiler that treat
	differently the object of the class which implemented marker interface.
...
Which methods are used during Serialization and DeSerialization process in Java?
  	We call ObjectOutputStream.writeObject(saveThisobject) to serialize and
  	...
  	from the persistence and to create object from those bytes and it returns an Object
...
What if we make static variable transient?
	Serialization is for objects. And Static variables are not part of object state.
  	So static variables are not serialized. Thus no use of using transient.
...
What if a member of class doesn't implement Serializable interface?
  	...
  	If you try to serialize an object of a class which implements Serializable, but the object includes
	a reference to a non- Serializable class then a 'NotSerializableException' will be thrown at runtime
  	whenever we are serializing an object, set of all objects which are reachable
  	from that object will be serialized automatically. This group of objects is called object graph
...
from super class after deserialization?
	If any variable is inherited from parent then jvm ignores the value and write the default value on file.
  	Deserialization:
  	JVM checks if the parent is serializable or not, if not then
...
What is Externalizable? the difference between Serializable and Externalizable interface in Java?⭐️
  	Externalizable is an interface.(Not Marker)
  		in serialization everything is taken care by jvm and programmer does not have any control.
  	...
  	In externalizable interface there are 2 methods:
