...
Why don't we need to Import java.lang Package in class?⭐️
	We don't need to Import java.lang Package. Because all the classes in this Package are imported by default.
...
Why java.lang.Object is the base class?
	Object Class contains the most common methods that are useful for any java object. i.e equals(), hashCode() etc.
	-> To make those available for every object, Java made every Class to be child Class of object directly or indirectly.
...
What are the common methods provided by object class?
	...
	clone(): creates and returns a copy of the object. (Cloneable interface should be implemented)
...
Is String a datatype?
	String in java is not a primitive data type like int, long, char. It is a Class or can be considered as user defined type.
...
How String is different from a char array?
	...
	4. String can be stored in memory in any way, while elements of character array are stored contiguously in increasing memory locations.
...
How these Statements are different?⭐️
	1. String str = "Basic";
	2. String strr = new String("Basic");
...
What is String Interning?⭐️⭐️
	...
	We should not use intern if you don't have too many duplicate values

What intern() method is used for?⭐️
	In Java, when we perform any operation using the intern() method, it returns a canonical representation for the string object.
	A pool is managed by a String class.

	1 When the intern() method is executed, it checks whether the String equals to this String Object is in the pool.
	2 If it is available, then the string from the pool is returned. Otherwise, this String object is added to the pool
	...
	It is advised to use equals(), not ==, to compare two strings. This is because the == operator compares memory locations,
	while the equals() method compares the content stored in two objects.
...
Can we create an immutable class?⭐️
	...
	-> By not providing any method to change the state of the object (dont define setters), we can create an immutable class.
...
Which of them is better to store password? String or Char array?
	...
	string are stored in String pool, so once its created it always stays in the pool until Garbage collector erase it.
	So even though we are done with password kind stuff, it will be available in memory for the very long duration.
	It is a security risk, because anyone who is having access to memory can find password easily.
...
Difference between String, StringBuffer, StringBuilder.⭐️
	...
	Whenever we do String manipulation, it generates a new String and discards the older String for garbage collection.
	...
	That makes its performance faster than StringBuffer.

	Both StringBuffer and StringBuilder has not overriden the equals method so equals of object class gets executed which compares references.
...
How to check if two strings are equal?⭐️
	...
	👉🏻 equals() method of object Class checks for references, but String has different implementation of equals() method.
	   It checks for the values of the string.
...
How to convert String to charArray and byteArray?
	To get char array from string we use toCharArray() method, & getBytes() is used to get bytes from a string.
...
What is the use of Math class?
	Math Class provides the mathematical functions. i.e. sqrt(), exp(), log(), abs().
...
What's Autoboxing and unboxing in java?⭐️⭐️
	Autoboxing : Introduced in Java 1.5 to automatically converts the primitive type into
...
When do Autoboxing and Unboxing occur?⭐️⭐️
	...
	For example,
	A method expect an object argument but we pass primitive. In that case java automatically converts primitive into equal value object.
